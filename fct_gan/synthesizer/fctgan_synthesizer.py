...
class CFNO2d(Module):
    def __init__(self, side, dim, drop_path=0.2, dropout=0.2, mlp_ratio = 4.0, depth = 4, embed_dim = 64):
        '''
...
class GFGenerator(Module):
    ...
    """
    def __init__(self, gside, base_width = 4, random_dim = 100, embed_dim = 64):
        super(GFGenerator, self).__init__()
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.target_side = gside
    ...
    def forward(self, z):
        ...
        if self.target_side != self.bottom_width * 4:
            output = F.interpolate(
                output,
                size=(self.target_side, self.target_side),
                mode="bilinear",
                align_corners=False
            )
